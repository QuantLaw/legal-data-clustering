...
def louvain(g, weight="weight", resolution=1.0, seed=None, return_tree=False):
    ...
    In the local moving phase, individual nodes are moved to the community that
    yields the largest increase in the quality function.
    In the aggregation phase, an aggregate network is created based on the
    partition obtained in the local moving phase.
    Each community in this partition becomes a node in the aggregate network.
    The two phases are repeated until the quality function cannot be increased
    further.

    :param g: a networkx/igraph object
    :param weight: str, optional the key in graph to use as weight.
                   Default to 'weight'.
    :param resolution: double, optional  Will change the size of the communities,
                       default to 1.
    :param randomize:  boolean, optional  Will randomize the node evaluation order
                       and the community evaluation order to get different partitions
                       at each call, default False
    :return: NodeClustering object
    ...
    <https://iopscience.iop.org/article/10.1088/1742-5468/2008/10/P10008/meta/>`_
...
def missings_nodes_as_additional_clusters(clustering: NodeClustering):
    """
    Copy the NodeClustering and add the nodes that were not covered by the
    original clustering, e.g., nodes that don't have any im_edges or out_edges.
